...
const waitForApiKeyPage = async () => {
    ...
    ]).catch(() => fail(page, '02-admin-api-page', 'API key page did not render (neither key row nor "Neuer Apikey" button found)'));
};
...
await page.locator(`tr:has-text("${KEY_NAME}")`).first().waitFor({ timeout: 15000 }).catch(() => {});
...
const openKeyModal = async () => {
    ...
    await page.waitForFunction(() => /^[A-Za-z0-9]{32}$/.test(document.querySelector('textarea.copy-text__textbox')?.value ?? ''), { timeout: 15000 })
...
const extractKey = async () => {
    const textbox = await page.locator('textarea.copy-text__textbox').first().inputValue().catch(() => '');
    ...
    let key = [
        ...
        ...(await Promise.all((await page.locator('input, textarea').all()).map((el) => el.inputValue().catch(() => '')))),
    ]
        .flatMap((text) => text?.match(/[A-Za-z0-9]{32}/g) ?? [])
        .find((t) => !textBeforeModal.includes(t));
    if (!key) {
        await page.getByText('Kopieren').first().click().catch(() => {});
        key = await page.evaluate(() => navigator.clipboard.readText().catch(() => ''))
...
if (!keyExists) {
    ...
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.locator(`tr:has-text("${KEY_NAME}")`).first().waitFor({ timeout: 10000 }).catch(() => {});
}
...
const realGroup =
    periodGroups.find((pg) => (pg.children?.debitorcategory ?? []).length > 0) ?? periodGroups[0];
...
const byType = (title, type) =>
    accounts.filter(
        (a) =>
            ...
            (a.parents ?? []).some(
                (gid) => groupById.get(gid)?.properties.type === type && groupById.get(gid)?.parents?.includes(period.id),
            ),
    );
...
const creditId = pick(byType('Einnahmen aus Veranstaltungen', 'income'), 'credit account "Einnahmen aus Veranstaltungen" (income)');

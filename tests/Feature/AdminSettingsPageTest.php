...
it('can update settings via the AdminSettings component and persists them', function (): void {
    ...
    $settings->api_url = 'https://initial.example/v1';
    ...
    $component
        ->set("values.$class.api_url", 'https://changed.example/v2')
        ->call('saveSingle', $class, 'api_url')
        ->call('commitPending');

    expect(app(WeblingApiSettings::class)->api_url)->toBe('https://changed.example/v2');
    ...
    $component
        ->set("values.$class.api_key", 'new-secret-key')
        ->call('saveSingle', $class, 'api_key')
        ->call('commitPending');

    expect(app(WeblingApiSettings::class)->api_key)->toBe('new-secret-key');

    // Component state refreshes after save
    $component->assertSet("values.$class.api_url", 'https://changed.example/v2');
});
...
it('validates invalid values and shows errors without saving', function (): void {
    ...
    /** @var WeblingApiSettings $settings */
    $settings = app(WeblingApiSettings::class);
    $settings->api_url = 'https://okay.example/v1';
    ...
    Livewire::test(AdminSettings::class)
        ->set("values.$class.api_url", 'not-a-url')
        ->call('saveSingle', $class, 'api_url')
        ->assertHasErrors(["values.$class.api_url" => 'url']);

    // Value should remain unchanged in persisted settings
    expect(app(WeblingApiSettings::class)->api_url)->toBe('https://okay.example/v1');
});

...
it('seeds local development graph with external users and two events', function (): void {
    ...
    expect(DonationEvent::query()->whereIn('slug', ['2025', '2026'])->count())->toBe(2)
        ...
        ->and(AthleteRegistration::query()->where('group_membership_status', 'accepted')->where('group_membership_role', 'admin')->count())->toBe(4)
        ->and(AthleteRegistration::query()->where('group_membership_status', 'accepted')->where('group_membership_role', 'member')->count())->toBe(1)
        ->and(AthleteRegistration::query()->where('group_membership_status', 'pending')->whereNull('group_membership_role')->count())->toBe(1)

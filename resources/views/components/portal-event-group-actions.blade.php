<div>
    ...
    @else
        ...
        @endphp
        <flux:button wire:click="confirm" :variant="in_array($action, ['deny', 'remove', 'delete'], true) ? 'danger' : 'outline'" size="sm">
            {{ $labels[$action] }}
        </flux:button>
        <flux:modal name="group-confirm-{{ $this->getId() }}" wire:model.self="confirming" :dismissible="false" class="space-y-6 sm:w-full md:w-xl">
            <div class="space-y-2">
            ...
            <div class="flex gap-2">
                ...
                <flux:button wire:click="submit" wire:target="submit" :variant="in_array($action, ['deny', 'remove', 'delete'], true) ? 'danger' : 'primary'">

    ...
    <div class="grid grid-cols-2 gap-3">
        @foreach (['light' => 'Hell', 'dark' => 'Dunkel'] as $variant => $label)
            <button
                ...
                type="button"
                class="focus-visible:outline-hfm-red data-[selected=true]:border-hfm-red overflow-hidden rounded-xl border-2 border-transparent text-left focus-visible:outline-2 focus-visible:outline-offset-2"
            >
                <div class="relative aspect-[9/16]">
                    <div
                        data-story-preview-skeleton="{{ $variant }}"
                        class="absolute inset-0 rounded-lg bg-zinc-200 transition-opacity duration-300 ease-out motion-reduce:transition-none dark:bg-zinc-700"
                    >
                    ...
                    <img
                        ...
                        alt="{{ $label }} Vorschau{{ $athleteName ? ' für '.$athleteName : ' deiner Story' }}"
                        class="absolute inset-0 size-full object-cover opacity-0 transition-opacity duration-300 ease-out motion-reduce:transition-none"
                    />
                </div>

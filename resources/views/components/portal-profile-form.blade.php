<flux:card class="border-hfm-light/40 space-y-8 rounded-xl bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
    <div>
    ...
    <form wire:submit="save" class="space-y-8">
        <div class="grid gap-6 sm:grid-cols-2">
            <flux:input
                :value="auth('external')->user()?->first_name"
                label="Vorname"
                icon-trailing="user"
                variant="filled"
                readonly
            />
            <flux:input
                :value="auth('external')->user()?->last_name"
                label="Nachname"
                icon-trailing="user"
                variant="filled"
                readonly
            />
            <flux:input
                :value="auth('external')->user()?->email"
                label="E-Mail"
            ...
            />
            <flux:input
                :value="match (auth('external')->user()?->country_of_residence) { 'DE' => 'Deutschland', 'AT' => 'Österreich', default => 'Schweiz' }"
                label="Wohnland"
                icon-trailing="globe-europe-africa"
                variant="filled"
                readonly
            />
            <flux:input
                :value="auth('external')->user()?->public_id_string"
                label="Öffentliche ID"
                icon-trailing="identification"
                variant="filled"
                readonly
            />
            ...
            <flux:callout icon="information-circle" variant="secondary" class="sm:col-span-2">
                <flux:callout.text>
                    Für Änderungen an deinem Namen, deiner E-Mail-Adresse oder deinem Wohnland kontaktiere uns bitte
                    über
                    <a href="{{ route('contact') }}" wire:navigate class="font-medium underline">Hilfe & Kontakt</a>.
                </flux:callout.text>
            </flux:callout>
                ...
                <flux:input.group style="display: flex; width: 100%">
                    <div data-flux-input style="flex: 0 0 33.333333%">
                        <flux:input
                            ...
                            :mask="auth('external')->user()?->country_of_residence === 'DE' ? '99999' : '9999'"
                            :placeholder="auth('external')->user()?->country_of_residence === 'DE' ? '57123' : '8406'"

...
- Keep lockstep ecosystems together (for example `vite` + `laravel-vite-plugin`; `tailwindcss` + related plugins).
...
- Run one focused verification pass per batch:
    - Frontend-heavy batch: `npm run build:ai` and `composer lint:ai`
...
- After each major step, run fast high-signal checks first:
    ...
    - Build (`npm run build:ai`) for frontend toolchain changes

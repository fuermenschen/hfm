...
Pulse::record("user_sale", $user->id, $sale->amount)->sum()->count();
```
...
class SaleRecorder
{
    public array $listen = [\App\Events\SaleCompleted::class];

    public function record(\App\Events\SaleCompleted $event): void
    {
        \Laravel\Pulse\Facades\Pulse::record("user_sale", $event->user->id, $event->sale->amount)->sum()->count();
    }
}
```
